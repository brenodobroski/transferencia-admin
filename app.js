...
function abrirModalDetalheVenda(id) {
  ...
  // ⬅ TIMELINE CRONOLÓGICA: respostas do admin + ajustes do lojista em ORDEM DE DATA
  ...
  const blocoAjust = a => `
    <div>
      <span class="text-slate-400 font-bold uppercase text-[9px] tracking-wide block mb-1">Ajuste do lojista</span>
      <div class="border border-slate-200 bg-white rounded-sm px-3 py-2 flex flex-col gap-1.5">
        ...
        ${(a.evidencias || []).map((ev, i) => `<p class="text-[11px] text-slate-500"><i class="fas fa-paperclip text-slate-300 mr-1"></i><a href="#" onclick="event.preventDefault(); baixarEvidenciaAdmin('${v.id}', ${JSON.stringify(a.data)}, ${i})" class="text-blue-700 font-bold">${escapeHtml(ev.nome)}</a></p>`).join("")}
      </div>
    </div>`;
...
function abrirModalNegarDefinitivo(id) {
  ...
  // ⬅ Reseta pra etapa 1 (digitar motivo), escondendo o aviso de confirmação
  $("etapa-motivo-def").classList.remove("hidden");
  $("etapa-confirmar-def").classList.add("hidden");
  $("modal-negar-definitivo").classList.remove("hidden");
  $("input-motivo-negacao-def").focus();
}

// ⬅ Etapa 2: mostra o aviso de confirmação na tela (SEM alert)
function mostrarConfirmacaoNegarDef() {
  ...
  $("msg-modal-negar-def").classList.add("hidden");
  $("etapa-motivo-def").classList.add("hidden");
  $("etapa-confirmar-def").classList.remove("hidden");
}

function voltarMotivoNegarDef() {
  $("etapa-confirmar-def").classList.add("hidden");
  $("etapa-motivo-def").classList.remove("hidden");
}
...
Object.assign(window, {
  ...
  mostrarConfirmacaoNegarDef, voltarMotivoNegarDef,
  mudarMes,
